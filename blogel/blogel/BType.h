...
vector<int> v2part, v2degree, v2bound, vFlag, v2degree_R, valMid; // 全局描述变量
...
vector<unsigned> v2p, p2v, v2p_Global, p2v_Global, v2root, root2v;
long long paths=0;
...
int CutEdge = 0, CopyNum = 0, SelfCopy = 0;
...
struct TriElem{
	unsigned dis, vid, flg;
	
	TriElem(){ dis = 0, vid = 0, flg = 0; }

	TriElem(unsigned d, unsigned v, unsigned p){
		dis = d, vid = v, flg = p;
	}
	...
	bool operator < (const TriElem& a) const{
		if (vid < a.vid)
			...
			if (dis < a.dis) 
				return true;
			else if (dis == a.dis and flg > a.flg){
				return true;
			}else
				return false;
		}else
...
public:
	IntVal(){x=-1;};
	IntVal(int _x){this->x=_x;};
	bool operator<(const IntVal &v) const {if(score[x] == score[v.x]) return x<v.x; return score[x] < score[v.x];};
};
